...
void TCP::Download(const std::string_view &sv, const byte_pair &chunk,
    ...
    const std::string chunk_range = "Range: bytes=" + chunk.first + "-" + chunk.second;
    ...
    while (true) {
        ...
        auto incoming = recv(fd, current_frame.buffer.data(), current_frame.buffer.size(), 0);
...
Frame TCP::Query() {
    ...
    while (recv(fd, current_frame.buffer.data(), current_frame.buffer.size(), 0) > 0)
