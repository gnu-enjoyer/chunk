...
struct Frame {
  ...
  void Write(std::basic_fstream<char>& stream, ssize_t len) {
    ...
    if (itr != buffer.end()) {
      ...
      stream.write((buffer.begin() + dist + 4), (len - dist - 4));
...
struct CLI {
  ...
  int GetPort() const { return std::stoi(port); }
  ...
  template <typename... Args> static std::optional<CLI> Parse(Args&&... args) {
    try {
